...
PlayerWriteServiceImpl::PlayerWriteServiceImpl(PlayerQueryService& player_query_service,
                                               PlayerWriteService& player_write_service)
    : player_query_service_(player_query_service),
      player_write_service_(player_write_service) {}

::grpc::Status PlayerWriteServiceImpl::GetPlayerSnapshot(
    ...
    const auto result = player_query_service_.GetPlayerSnapshot(request->player_id());
    if (!result.success) {
...
::grpc::Status PlayerWriteServiceImpl::InvalidatePlayerCache(
    ::grpc::ServerContext* /*context*/,
    ...
    const auto result = player_query_service_.InvalidatePlayerCache(request->player_id());
    if (!result.success) {
        return ::grpc::Status(::grpc::StatusCode::INTERNAL, result.error_message);
    }
...
::grpc::Status PlayerWriteServiceImpl::GetBattleEntrySnapshot(
    ::grpc::ServerContext* /*context*/,
    ...
    const auto result = player_write_service_.GetBattleEntrySnapshot(request->player_id());
    if (!result.success) {
        return ::grpc::Status(::grpc::StatusCode::INTERNAL, result.error_message);
    }
    response->set_found(result.found);
...
::grpc::Status PlayerWriteServiceImpl::PrepareBattleEntry(
    ::grpc::ServerContext* /*context*/,
    ...
    const auto result = player_write_service_.PrepareBattleEntry(
...
::grpc::Status PlayerWriteServiceImpl::CancelBattleEntry(
    ::grpc::ServerContext* /*context*/,
    ...
    const auto result = player_write_service_.CancelBattleEntry(
...
::grpc::Status PlayerWriteServiceImpl::ApplyRewardGrant(
    ::grpc::ServerContext* /*context*/,
    ...
    const auto result = player_write_service_.ApplyRewardGrant(
    ...
    for (const auto& currency : result.applied_currencies) {
        auto* applied_reward = response->add_rewards();
        applied_reward->set_reward_type(currency.currency_type);
        applied_reward->set_amount(currency.amount);
    }
    return ::grpc::Status::OK;
}
