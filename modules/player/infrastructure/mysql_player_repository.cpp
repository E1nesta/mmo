...
std::optional<common::model::PlayerState> MySqlPlayerRepository::LoadPlayerState(std::int64_t player_id) const {
    ...
    for (const auto& row : progress_rows) {
        state.stage_progress.push_back({std::stoi(row.at("stage_id")),
                                        std::stoi(row.at("best_star")),
                                        row.at("is_first_clear") != "0"});
    }
...
std::string MySqlPlayerRepository::StageProgressTable() {
