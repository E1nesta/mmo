...
// 状态读取：`GetDungeonEntrySnapshot` 负责加载上下文并返回稳定结果。
GetDungeonEntrySnapshotPortResponse GrpcPlayerSnapshotPort::GetDungeonEntrySnapshot(std::int64_t player_id) const {
    ...
    for (const auto& role_summary : response.role_summaries()) {
        snapshot.role_summaries.push_back(
            DungeonRoleSummary{role_summary.role_id(), role_summary.level(), role_summary.star()});
    }
    return {true, true, common::error::ErrorCode::kOk, "", snapshot};
}

// 状态推进：`InvalidatePlayerSnapshot` 执行写链或补偿并收敛状态变化。
...
// 状态推进：`PrepareDungeonEntry` 执行写链或补偿并收敛状态变化。
PrepareDungeonEntryPortResponse GrpcPlayerSnapshotPort::PrepareDungeonEntry(std::int64_t player_id,
...
CancelDungeonEntryPortResponse GrpcPlayerSnapshotPort::CancelDungeonEntry(std::int64_t player_id,
                                                                        std::int64_t session_id,
                                                                        int energy_refund,
                                                                        const std::string& idempotency_key) {
