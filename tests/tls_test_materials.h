#pragma once

#include <filesystem>
#include <fstream>
#include <string>

namespace test::tls {

inline constexpr char kCaPem[] =
R"(-----BEGIN CERTIFICATE-----

gdiWBHAVrrpW
-----END CERTIFICATE-----
)";

inline constexpr char kServerCertPem[] =
R"(-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)";

inline constexpr char kServerKeyPem[] =
R"(-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
)";

inline constexpr char kWrongCaPem[] =
R"(-----BEGIN CERTIFICATE-----

15W02eb86VAb7WExZkIPDtsunf8s
-----END CERTIFICATE-----
)";

inline bool WriteFile(const std::filesystem::path& path, const std::string& content) {
    std::ofstream output(path, std::ios::binary);
    if (!output.is_open()) {
        return false;
    }
    output << content;
    return output.good();
}

inline bool WriteFixtureSet(const std::filesystem::path& directory) {
    const auto write = [&directory](const std::string& name, const std::string& content) {
        return WriteFile(directory / name, content);
    };

    return write("ca.pem", kCaPem) &&
           write("wrong-ca.pem", kWrongCaPem) &&
           write("server.crt", kServerCertPem) &&
           write("server.key", kServerKeyPem) &&
           write("gateway.crt", kServerCertPem) &&
           write("gateway.key", kServerKeyPem) &&
           write("online_gateway_server.crt", kServerCertPem) &&
           write("online_gateway_server.key", kServerKeyPem) &&
           write("auth_server.crt", kServerCertPem) &&
           write("auth_server.key", kServerKeyPem) &&
           write("player_query_server.crt", kServerCertPem) &&
           write("player_query_server.key", kServerKeyPem) &&
           write("dungeon_runtime_server.crt", kServerCertPem) &&
           write("dungeon_runtime_server.key", kServerKeyPem) &&
           write("social_server.crt", kServerCertPem) &&
           write("social_server.key", kServerKeyPem) &&
           write("player_write_grpc_server.crt", kServerCertPem) &&
           write("player_write_grpc_server.key", kServerKeyPem) &&
           write("login_server.crt", kServerCertPem) &&
           write("login_server.key", kServerKeyPem) &&
           write("player_server.crt", kServerCertPem) &&
           write("player_server.key", kServerKeyPem) &&
           write("battle_server.crt", kServerCertPem) &&
           write("battle_server.key", kServerKeyPem) &&
           write("player_internal_grpc_server.crt", kServerCertPem) &&
           write("player_internal_grpc_server.key", kServerKeyPem);
}

}  // namespace test::tls
