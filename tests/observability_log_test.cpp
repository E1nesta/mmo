...
int main() {
    ...
    context.message_id = common::net::MessageId::kPlayerInitRequest;
    ...
    if (!Expect(rendered.find("message_id=PLAYER_INIT_REQUEST") != std::string::npos, "expected message_id field")) {
        return 1;
    }
    ...
    if (!Expect(rendered.find("upstream.service=player_query") != std::string::npos, "expected upstream service field")) {
        return 1;
    }
