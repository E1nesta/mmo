...
int main() {
    ...
    // Build a config-backed in-memory account repo to exercise the same config loading path as auth bootstrap.
    ...
    const auto login_success = login_service.Login({"demo", "demo123", "127.0.0.1", "device-login-test"});
    ...
    if (!Expect(login_success.session.device_id == "device-login-test",
                "expected session device id to be bound on login")) {
        return 1;
    }
