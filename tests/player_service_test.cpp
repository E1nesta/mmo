...
int main() {
    ...
    if (!Expect(first_load.home_init.dungeon_summary.main_stage_id == 1001 &&
                    first_load.home_init.dungeon_summary.main_chapter_id == 1 &&
                    first_load.home_init.character_summary.fight_power == 1200 &&
                    first_load.home_init.player_display.nickname == "hero_demo" &&
                    first_load.home_init.dungeon_summary.stage_progress.size() == 1 &&
                    first_load.home_init.character_summary.role_summaries.size() == 3 &&
                    !first_load.home_init.activity_summary.hydrated &&
                    !first_load.home_init.entry_state_summary.hydrated,
                "expected load response to include home aggregation fields")) {
        return 1;
    }

    const auto player_snapshot = player_service.GetPlayerSnapshot(20001);
    if (!Expect(player_snapshot.success && player_snapshot.found && player_snapshot.nickname == "hero_demo" &&
                    player_snapshot.main_stage_id == 1001 && player_snapshot.main_chapter_id == 1 &&
                    player_snapshot.fight_power == 1200 && player_snapshot.stage_progress_count == 1 &&
                    player_snapshot.role_summaries.size() == 3 && player_snapshot.currencies.size() == 2,
                "expected player snapshot to include summarized home fields")) {
        return 1;
    }
    ...
    if (!Expect(reloaded.home_init.resource_summary.stamina == 110 &&
                    reloaded.home_init.resource_summary.gold == 1100 &&
                    reloaded.home_init.resource_summary.diamond == 150,
    ...
    if (!Expect(fallback_reload.home_init.resource_summary.stamina == 110,
