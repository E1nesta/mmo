...
bool WriteConfig(const std::filesystem::path& path,
    ...
    output << "service.name=online_gateway_server\n";
    output << "service.instance_id=online_gateway_test\n";
    output << "runtime.environment=dev\n";
    output << "service.listen.host=127.0.0.1\n";
    output << "service.listen.port=7000\n";
    output << "runtime.shutdown_grace_ms=5000\n";
    output << "transport.io_threads=1\n";
    output << "transport.idle_timeout_ms=30000\n";
    output << "transport.write_queue_limit=128\n";
    output << "transport.max_packet_body_bytes=4194304\n";
    ...
    output << "storage.redis.password=\n";
    output << "storage.redis.timeout_ms=100\n";
    output << "storage.redis.pool_size=1\n";
    output << "storage.session.ttl_seconds=3600\n";
    output << "execution.worker_threads=1\n";
    output << "execution.shard_count=1\n";
    output << "execution.queue_limit=1024\n";
...
int main(int /*argc*/, char* argv[]) {
    ...
    if (!Expect(redis_port > 0, "expected closed dependency port to be reserved")) {
        return 1;
    }
