...
void LogProxyProtocolFailure(std::uint64_t connection_id,
                             std::string_view peer_address,
                             std::string_view detail) {
    framework::observability::LogEntry entry;
    entry.Add("event", "proxy_protocol_invalid");
    entry.Add("connection_id", connection_id);
    entry.Add("peer.address", std::string(peer_address));
    entry.Add("detail", std::string(detail));
    common::log::Logger::Instance().Log(common::log::LogLevel::kWarn, entry.Build());
}
...
struct TransportServer::Impl {
    ...
    struct SessionBase {
        ...
        virtual void SendPacket(const common::net::Packet& packet) = 0;
    };
...
public:
    ...
    void SendPacket(const common::net::Packet& packet) override {
        auto self = this->shared_from_this();
        boost::asio::post(strand_, [self, packet] {
            if (self->closed_ || self->shutting_down_ || self->impl_.stopping.load()) {
                return;
            }
...
private:
    void StartReadLoop() {
        if (impl_.options.proxy_protocol_enabled && !proxy_protocol_parsed_) {
            ReadProxyProtocolHeader();
            return;
        }
    ...
    void ReadProxyProtocolHeader() {
        if (closed_ || shutting_down_ || impl_.stopping.load() || read_in_progress_) {
            return;
        }
        ...
        boost::asio::async_read(
            LowestLayer(stream_),
            boost::asio::buffer(&proxy_header_byte_, 1),
            boost::asio::bind_executor(
                strand_,
                [self](const boost::system::error_code& error, std::size_t /*bytes*/) {
                    self->OnProxyProtocolHeaderByte(error);
                }));
    }

    void OnProxyProtocolHeaderByte(const boost::system::error_code& error) {
        if (error) {
        ...
        proxy_protocol_header_.push_back(proxy_header_byte_);
        if (proxy_protocol_header_.size() > kMaxProxyProtocolHeaderBytes) {
            read_in_progress_ = false;
            LogProxyProtocolFailure(connection_id_, peer_address_, "proxy protocol header exceeds max size");
            Close();
            return;
        }

        RefreshDeadline();
        if (proxy_protocol_header_.size() >= 2 &&
            proxy_protocol_header_[proxy_protocol_header_.size() - 2] == '\r' &&
            proxy_protocol_header_[proxy_protocol_header_.size() - 1] == '\n') {
            read_in_progress_ = false;
            proxy_protocol_parsed_ = true;

            std::string proxied_peer_address;
            std::string error_message;
            if (!ParseProxyProtocolHeader(proxy_protocol_header_, &proxied_peer_address, &error_message)) {
                LogProxyProtocolFailure(connection_id_, peer_address_, error_message);
                Close();
                return;
            }
            if (!proxied_peer_address.empty()) {
                peer_address_ = std::move(proxied_peer_address);
            }
    ...
    std::string proxy_protocol_header_;
    std::array<char, framework::protocol::kPacketHeaderSize> read_header_{};
    std::string read_body_;
    std::deque<std::string> write_queue_;
    char proxy_header_byte_ = '\0';
    bool proxy_protocol_parsed_ = false;
...
bool TransportServer::SendToConnection(std::uint64_t connection_id, const common::net::Packet& packet) {
    std::shared_ptr<Impl::SessionBase> session;
    {
        std::lock_guard lock(impl_->sessions_mutex);
        const auto iter = impl_->sessions.find(connection_id);
...
bool TransportServer::CloseConnection(std::uint64_t connection_id) {
    std::shared_ptr<Impl::SessionBase> session;
    {
        std::lock_guard lock(impl_->sessions_mutex);
        const auto iter = impl_->sessions.find(connection_id);
