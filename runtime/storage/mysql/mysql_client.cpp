...
ConnectionOptions ReadConnectionOptions(const config::SimpleConfig& config, const std::string& prefix) {
    if (!HasEndpointConfig(config, prefix) && HasEndpointConfig(config, prefix + "writer.")) {
        return ReadConnectionOptions(config, prefix + "writer.");
    }
...
std::size_t ReadPoolSize(const config::SimpleConfig& config, const std::string& prefix, std::size_t default_value) {
    if (!config.Contains(prefix + "pool_size") && HasEndpointConfig(config, prefix + "writer.")) {
        return ReadPoolSize(config, prefix + "writer.", default_value);
    }
...
ReadWritePoolOptions ReadReadWritePoolOptions(const config::SimpleConfig& config,
    ...
    ReadWritePoolOptions options;
    const auto writer_prefix = base_prefix + "writer.";
    const auto reader_prefix = base_prefix + "reader.";

    options.writer.connection = ReadConnectionOptionsWithFallback(config, writer_prefix, base_prefix);
    options.writer.pool_size = ReadPoolSizeWithFallback(config, writer_prefix, base_prefix, default_pool_size);

    if (HasEndpointConfig(config, reader_prefix)) {
        PoolOptions reader;
        reader.connection = ReadConnectionOptions(config, reader_prefix);
        reader.pool_size = ReadPoolSize(config, reader_prefix, options.writer.pool_size);
        options.reader = std::move(reader);
    }
