...
bool ShardedRequestExecutor::Submit(const ExecutionKey& key,
                                    ...
                                    SubmitFailureCode* failure_code) {
    if (failure_code != nullptr) {
        *failure_code = SubmitFailureCode::kNone;
    }
    if (!started_.load()) {
        if (error_message != nullptr) {
        ...
        if (failure_code != nullptr) {
            *failure_code = SubmitFailureCode::kNotStarted;
        }
        return false;
    }
    ...
    if (!accepting_.load()) {
        ...
        if (failure_code != nullptr) {
            *failure_code = SubmitFailureCode::kStopping;
        }
        return false;
    }

    if (!TryReserveQueueSlot(error_message, failure_code)) {
        return false;
    }
    ...
    while (true) {
        ...
        try {
            ...
            framework::observability::EventLogBuilder(
                framework::observability::LogEvent::kRequestExecutorTaskFailed)
                .WithErrorCode(framework::observability::LogErrorCode::kRequestDispatchFailed)
                .WithDetail(exception.what())
                .Emit(common::log::LogLevel::kError);
        } catch (...) {
            framework::observability::EventLogBuilder(
                framework::observability::LogEvent::kRequestExecutorTaskFailed)
                .WithErrorCode(framework::observability::LogErrorCode::kRequestDispatchFailed)
                .WithDetail("unknown exception")
                .Emit(common::log::LogLevel::kError);
        }
        ReleaseQueueSlot();
    }
...
bool ShardedRequestExecutor::TryReserveQueueSlot(std::string* error_message, SubmitFailureCode* failure_code) {
    ...
    while (true) {
        if (current >= options_.queue_limit) {
            ...
            if (failure_code != nullptr) {
                *failure_code = SubmitFailureCode::kQueueLimitExceeded;
            }
            return false;
        }
