...
constexpr std::size_t kRpcHeaderBytes =
    ...
    RpcFrameCodec::kRpcFrameOverheadBytes;
...
bool decode_batch_body(
    ...
    if (count > (data.size() - offset) / sizeof(std::uint32_t)) {
        set_error(error_message, "rpc batch count exceeds encoded body");
        return false;
    }
...
std::vector<std::uint8_t> RpcFrameCodec::encode_rpc_frame(
    const runtime::protocol::FrameMessage& frame,
    std::uint32_t max_payload_bytes,
    std::string* error_message) {
...
bool RpcFrameCodec::decode_rpc_frame(
    const std::vector<std::uint8_t>& data,
    std::uint32_t max_payload_bytes,
    runtime::protocol::FrameMessage* frame,
    std::string* error_message) {
    ...
    if (!runtime::protocol::is_valid_message_mode(decoded.header.mode)) {
        set_error(error_message, "rpc frame mode is invalid");
        return false;
    }
