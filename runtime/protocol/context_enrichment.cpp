...
bool ExtractPlayerIdFromBody(common::net::MessageId message_id,
    ...
    case common::net::MessageId::kPlayerSnapshotRequest: {
        ...
        if (!common::net::ParseMessage(packet.body, &request)) {
            if (error_message != nullptr) {
                *error_message = "invalid player snapshot request";
            }
            return false;
        }
        *player_id = request.player_id();
    ...
    case common::net::MessageId::kGetActiveDungeonRequest: {
        ...
        if (!common::net::ParseMessage(packet.body, &request)) {
            if (error_message != nullptr) {
                *error_message = "invalid get active dungeon request";
            }
            return false;
        }
        *player_id = request.player_id();
    ...
    case common::net::MessageId::kGetSettlementGrantStatusRequest: {
        ...
        if (!common::net::ParseMessage(packet.body, &request)) {
            if (error_message != nullptr) {
                *error_message = "invalid get settlement grant status request";
            }
            return false;
        }
        *player_id = request.player_id();
