...
EXPECTED_MODULE_SHAPES = {
    ...
    "dungeon_runtime": {"application", "domain", "infrastructure", "ports"},
    "social": {"application", "domain", "infrastructure", "ports"},
}
...
def main() -> int:
    ...
    for path in source_files:
        ...
        for include in includes:
            ...
            if parts[0] == "apps":
                app_name = parts[1]
                if app_name in {"gateway", "online_gateway", "api_gateway"}:
                    if include.startswith("modules/"):
                        fail(errors, f"{relative}: gateway app 不应直接依赖业务模块 -> {include}")
                    continue

                if app_name == "player_internal_grpc_server":
                    owned_module = "player"
                elif app_name == "player_write_grpc_server":
                    owned_module = "player"
                elif app_name == "player_query":
                    owned_module = "player"
                elif app_name == "auth":
                    owned_module = "login"
                elif app_name == "dungeon_runtime":
                    owned_module = "dungeon_runtime"
                else:
