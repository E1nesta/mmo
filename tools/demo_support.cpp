...
std::vector<DemoRoleConfig> EffectiveRoles(const DemoDataConfig& config) {
    if (!config.roles.empty()) {
        return config.roles;
    }

    std::vector<DemoRoleConfig> roles;
    roles.reserve(kDefaultRoleIds.size());
    for (const auto role_id : kDefaultRoleIds) {
        roles.push_back({role_id, config.level, 1, 0});
    }
    return roles;
}
...
bool EnsureRoles(common::mysql::MySqlClient& player_mysql,
                 const DemoDataConfig& config,
                 std::string* error_message) {
    const auto roles = EffectiveRoles(config);
    std::ostringstream sql;
    ...
    for (std::size_t index = 0; index < roles.size(); ++index) {
        ...
        sql << "(" << config.player_id << ", " << roles[index].role_id << ", " << roles[index].level << ", "
            << roles[index].star << ", " << roles[index].breakthrough << ", NULL, NULL, CURRENT_TIMESTAMP(3))";
    }
    return ExecuteSql(player_mysql, sql.str(), error_message);
}
...
bool ResetPlayerState(common::mysql::MySqlClient& player_mysql,
    ...
    for (std::size_t index = 0; index < roles.size(); ++index) {
        ...
        role_sql << "(" << config.player_id << ", " << roles[index].role_id << ", " << roles[index].level << ", "
                 << roles[index].star << ", " << roles[index].breakthrough << ", NULL, NULL, CURRENT_TIMESTAMP(3))";
    }
...
DemoDataConfig ReadDemoDataConfig(const common::config::SimpleConfig& login_config,
    ...
    config.roles = EffectiveRoles(config);
    return config;
}
